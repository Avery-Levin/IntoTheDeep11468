    ...
    override fun runOpMode() {
        ...
        try {
            ...
            while (centerobj == null) {
                centerobj = findClosest(
                    overheadProcessor.lastDetection,
                    overheadProcessor.width,
                    overheadProcessor.height
                )
            }
            ...
            cameraDataToRealPosition(
                ...
                center.x,
                center.y
            ) { posX, posY, realAngleX, realAngleY, camWidthAngle, camHeightAngle ->
                run {
                    //
                    telemetry.addData(
                        "endData",
                        "x: $posX, y: $posY, raX:$realAngleX, raY:$realAngleY, cWa(x):$camWidthAngle, cHa(y):$camHeightAngle , heading: ${centerobj.getAngle()}"
                    )
                    telemetry.update()
                    //Thread.sleep(25000)
                    ActionSet(
